...
public:
	...
	Node* findBestNodeWithUCT(Node* node, int level)
	{
		double tmp;
		int parentVisit = (*node).getNodeVisitCount();
		Node* child = (*node).getNodeChild(0);
		double max;
		int player = child->getNodeState().getStatePlayerNo();
		Position p = child->getNodeState().getMove();
		if (level < 54)
		{
			double result = Valueprobabilities(player,p,0,child->getNodeState().getStateboard());
			max = result * uctValue(parentVisit, (*node).getChildScore(0), (*child).getNodeVisitCount());
			//max = probability[child->getNodeState().getMove().getX()][child->getNodeState().getMove().getY()] * uctValue(parentVisit, (*node).getChildScore(0), (*child).getNodeVisitCount());
		}
