...
/*
// local check 
bool checkVaild(node now,int num_TNT)
{
	// global check
			...
			{	// local check 8 directions 周圍的TNT 
				...
				for(int k = 0; k < 8 ; k ++){
					if(isVaild(i + dir[k].x,j +dir[k].y))
						if(now.value[i + dir[k].x][j +dir[k].y ] == 1)
							TNT ++;
				}
				if(TNT > mine[i][j])
					return false;
...
int main()
{
	...
	//----  forward checking with Degree heuristic
	cout<<"forward checking with Degree heuristic\n";
	start = clock();
	...
	//----  forward checking with uesr-defined heuristic
	cout<<"forward checking with TNT heuristic\n";
	start = clock();
	...
	//----  forward checking with uesr-defined heuristic
	cout<<"forward checking with MRV heuristic\n";
	start = clock();
